...
def resnet_v2(curr_image_inputs,hist_image_inputs,blocks,num_classes=None,global_pool=True,include_root_block=True,reuse=None,scope=None):
    end_points_collection='resnetv2_end_points'
    with tf.variable_scope(scope,'resnet_v2',[curr_image_inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d,bottleneck,stack_blocks_dense], outputs_collections= end_points_collection):
            ...
            if include_root_block:
                with slim.arg_scope([slim.conv2d],activation_fn=None,normalizer_fn=None):
                    net_curr=conv2d_same(net_curr,75,7,stride=2,scope='conv1')
...
#             end_points_curr['prediction']=slim.softmax(net_curr,scope='predictions')       
    ...
    with tf.variable_scope(scope,'resnet_v2',[hist_image_inputs], reuse=reuse) as sc:
#         end_points_collection_hist=sc.original_name_scope+'_end_points'
        with slim.arg_scope([slim.conv2d,bottleneck,stack_blocks_dense], outputs_collections= end_points_collection):
            ...
            if include_root_block:
                with slim.arg_scope([slim.conv2d],activation_fn=None,normalizer_fn=None):
                    net_hist=conv2d_same(net_hist,75,7,stride=2,scope='conv1')
...
def forward_propagation(curr_image_inputs,hist_image_inputs,is_training=True):
    with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
        return resnet_v2_50(curr_image_inputs,hist_image_inputs,ct.CLASS_NUM,reuse=tf.AUTO_REUSE)
