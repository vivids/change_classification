        ...
        with slim.arg_scope([slim.conv2d,bottleneck,stack_blocks_dense], outputs_collections= end_points_collection):
            ...
            if include_root_block:
                with slim.arg_scope([slim.conv2d],activation_fn=None,normalizer_fn=None):#slim.batch_norm
                    net=conv2d_same(net,75,7,stride=2,scope='conv1')

#                     net=slim.conv2d(net,25,7,stride=2,scope='conv1',activation_fn=tf.nn.relu)
#                     net=slim.conv2d(net,50,3,stride=1,scope='conv2',activation_fn=tf.nn.relu)
#                     net=slim.conv2d(net,75,3,stride=1,scope='conv3',activation_fn=None)
#                     net=slim.conv2d(net,75,7,stride=2,scope='conv1',activation_fn=tf.nn.relu)
#                     net=slim.conv2d(net,75,3,stride=1,scope='conv2',activation_fn=None)    
...
def foward_propagation(inputs,is_training=True):
    with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
        net, end_points=resnet_v2_50(inputs,ct.CLASS_NUM)
